...
export async function POST(request) {
  ...
  if (errors) return NextResponse.json({ error: "Email and password required" }, { status: 400 });
  ...
  const response = NextResponse.json({ userId: user._id, name: user.name, role: user.role });
