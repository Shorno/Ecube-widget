...
export default function OverallRanking() {
  ...
  const tournamentID = Array.isArray(rawTournamentID) ? rawTournamentID[0] : rawTournamentID;
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (ev === "MATCH_LIVE_RANK_DATA" && Array.isArray(data)) {
        if (!wasTopFourRef.current && containerRef.current) flipStateRef.current = Flip.getState(".team-row");
        setTeams([...data]);
      }
      if (ev === "SET_OBSERVING_PLAYER") setObservingTeamId(data?.player?.teamId ?? null);
    };
    ws.onerror = () => {};
  ...
  useEffect(() => {
    const aliveTeams = teams.filter((t) => !t.players.every((p) => p.liveState === 5));
    ...
    if (isTopFour && !wasTopFourRef.current) {
      ...
      if (listPanelRef.current) {
        gsap.to(listPanelRef.current, { x: "110vw", duration: 0.5, ease: "power2.in", onComplete: () => setShowTopFour(true) });
      } else setShowTopFour(true);
    }
    if (wasTopFourRef.current && showTopFour) {
      const liveById = Object.fromEntries(teams.map((t) => [t.team._id, t]));
      setTopFourTeams((prev) => prev.map((frozen) => { const live = liveById[frozen.team._id]; if (!live) return frozen; return { ...frozen, players: live.players, winProbability: live.winProbability ?? frozen.winProbability }; }));
    }
  }, [teams, showTopFour]);
  ...
  useLayoutEffect(() => {
    ...
    if (flipStateRef.current) { Flip.from(flipStateRef.current, { duration: 0.8, ease: "power3.inOut", stagger: 0.05 }); flipStateRef.current = null; }
  }, [teams, showTopFour]);
      ...
      {!showTopFour && teams.length > 0 && (
        <div ref={listPanelRef} className="fixed right-4 bottom-4 w-full max-w-100">
          <div className="grid grid-cols-7 border-b border-white bg-linear-to-r from-blue-900 via-blue-400 to-blue-900 text-sm text-white">
            {tableHeader.map((h) => (
              <div key={h.key} className={cn("p-2 text-center", h.label === "TEAM" && "col-span-3 text-left")}>{h.label}</div>
            ))}
          </div>
          <div ref={containerRef} className="relative flex flex-col bg-slate-900">
            {teams.map((entry, index) => (
              <TeamRow key={entry.team._id} entry={entry} isObserved={observingTeamId === entry.team._id} isOverall rank={index + 1} />
            ))}
          </div>
