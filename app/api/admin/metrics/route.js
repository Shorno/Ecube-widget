...
export async function GET(request) {
  ...
  if (view === "raw") {
    return NextResponse.json({ entries: getRaw().slice(0, 200), source: "memory" });
  }
  ...
  if (view === "history") {
    ...
    const sinceMs = { "1h": 3600000, "6h": 21600000, "24h": 86400000, "7d": 604800000, "30d": 2592000000 };
    ...
    const [grouped, sseSummary] = await Promise.all([
      ...
      Metric.aggregate([
        ...
        { $project: {
          ...
          method: "$_id.method",
          type:   "$_id.type",
          count:  1, avg: 1, min: 1, max: 1, errors: 1, lastTs: 1,
          // p50 and p95 from sorted array
          p50: { $arrayElemAt: ["$durations", { $floor: { $multiply: [{ $size: "$durations" }, 0.5] } }] },
          p95: { $arrayElemAt: ["$durations", { $floor: { $multiply: [{ $size: "$durations" }, 0.95] } }] },
        }},
        { $sort: { count: -1 } },
      ]),
