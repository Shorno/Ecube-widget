...
export function str(val, { min = 1, max = 500, optional = false } = {}) {
  ...
  const t = val.trim();
  if (!optional && t.length < min) return `Must be at least ${min} character${min !== 1 ? "s" : ""}`;
...
export function email(val) {
  ...
  if (val.includes("@") && !EMAIL_RE.test(val.trim())) return "Invalid email address";
  return null;
}
...
export function colorVal(val, { optional = true } = {}) {
  if (val === undefined || val === null || val === "") return optional ? null : "Required";
...
export function strObj(val, { maxKeys = 200, maxValLen = 200 } = {}) {
  if (!val || typeof val !== "object" || Array.isArray(val)) return "Must be an object";
