...
export function logRequest({ route, method, status, durationMs, extra = {} }) {
  ...
  return persist({ route, method, status, durationMs, type: "api_request", meta: extra, ts: new Date() });
}
 ...
 */
export function logSSECommand({ tournamentId, url, label, durationMs, broadcastMs }) {
  record({ route: "/api/sse/command", method: "POST", status: 200, durationMs });
  return persist({
