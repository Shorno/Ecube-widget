...
export function TeamRow({ entry, isObserved = false, isOverall = false, rank }) {
  ...
  const hasBlueZone = entry.players.some((p) => p.isOutsideZone && p.liveState !== 5);

  return (
    <div
      className={cn("team-row relative grid grid-cols-7 items-center border-b border-blue-900/30 bg-blue-100 text-sm text-white", eliminated && "opacity-90")}
      data-flip-id={entry.team._id}
    >
      <div className={cn("flex items-center justify-center bg-blue-700 p-2 font-bold text-white", isObserved && "bg-yellow-500 text-black")}>
        {rank}
      </div>
      <div className={cn("col-span-3 flex items-center gap-2 bg-blue-900 p-2", isObserved && "bg-yellow-700 text-black")}>
        <img src={entry.team.logoImageUrl} alt={entry.team.name} width={20} height={20} className="rounded" />
        <span className="truncate font-semibold uppercase">{entry.team.name}</span>
      </div>
      ...
      {hasBlueZone && (
        <div className="pointer-events-none absolute inset-0 z-50 animate-pulse" style={{ background: "radial-gradient(ellipse at center, rgba(147,197,253,0.5) 100%, rgba(59,130,246,0.5) 100%, transparent 100%)" }} />
      )}
    </div>
  );
}
