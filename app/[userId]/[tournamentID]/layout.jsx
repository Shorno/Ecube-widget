...
export default async function TournamentLayout({ children, params }) {
  ...
  const fontKey   = user.tournamentFonts?.[tournamentID] ?? user.themeConfig?.font ?? "oswald";
  ...
  const effectiveColors = user.tournamentColors?.[tournamentID] ?? user.themeConfig?.colors ?? {};
