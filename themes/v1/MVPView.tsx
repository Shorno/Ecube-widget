...
export default function MVPView({ tournamentID }: { tournamentID: string }) {
  ...
  console.log("MVP Data:", mvp);

  return (
    <WidgetStage dataReady={ready} onReady={() => setStageReady(true)}>
      <Layout top className="bg-red-900">
        <Title title="MATCH MVP" data={info} />
        <div className="mt-16 flex shrink-0 gap-26" ref={containerRef}>
...
const MVPStatsBox = ({
  ...
  return (
    <div className="relative flex w-140 gap-0">
      <div className="w-60 grid place-content-center text-widget-text-3">LOGO LEtter</div>
      <div className="bg-widget-bg font-primary text-widget-text-2 grid h-full w-full place-content-center text-[100px]">
        {value}
      </div>
