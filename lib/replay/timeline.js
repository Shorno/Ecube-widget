...
// PUBG-style zone countdown, straight from the setcircleinfo phase machine.
// Status 0 (hold): the blue zone is parked and the next white circle is already
// shown — count down to when it STARTS closing ("circle closing in"). Status 2
// (shrink): the blue zone is actively closing onto that circle — count down to
// when it finishes, which is the moment the next new circle is revealed ("new
// circle in"). Both countdowns are MaxTime - Counter; only the phase differs.
// Counter only ticks once per second, so anchor to the event's capture time —
// the countdown then stays smooth at any playback speed. Returns null whenever
// no circle timer should show.
export function circleTimerAt(timeline, time) {
  ...
  const elapsed = Number(Counter) + (time - info.t) / 1000;
