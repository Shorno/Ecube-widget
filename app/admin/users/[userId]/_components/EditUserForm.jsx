...
export default function EditUserForm({ user, designs = [] }) {
  ...
  const [allowedDesignIds, setAllowedDesignIds] = useState(
    user.allowedDesignIds?.length ? user.allowedDesignIds : [user.themeConfig?.designVariant ?? "default"]
  );
  // activeDesign = currently rendered design (themeConfig.designVariant)
  const [activeDesign, setActiveDesign] = useState(user.themeConfig?.designVariant ?? "default");
  ...
  const [tournamentNames, setTournamentNames] = useState(user.tournamentNames ?? {});
  ...
  function removeTid(tid) {
    ...
    setTournamentNames((p) => { const n = { ...p }; delete n[tid]; return n; });
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) { toast.error(data.error || "Save failed", { id }); return; }
  ...
  const subColor = daysLeft === null ? "text-gray-500"
    : daysLeft < 0  ? "text-red-400"
    : daysLeft <= 7 ? "text-amber-400"
    : "text-emerald-400";
  const subLabel = daysLeft === null ? "No subscription"
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
<Button onClick={() => setDeleteOpen(true)} disabled={deleting} variant="ghost" size="sm"
            className="text-gray-600 hover:bg-red-950/40 hover:text-red-400">
            Delete
          </Button>
          <Button onClick={save} disabled={saving}
            className="bg-gradient-to-r from-violet-600 to-pink-600 px-5 font-semibold text-white hover:from-violet-500 hover:to-pink-500">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
        ...
        <div className="space-y-5 lg:col-span-2">
            ...
            <div className="flex items-center justify-between rounded border border-gray-800 bg-gray-950/50 px-4 py-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  {isActive ? "Can access controller & widgets" : "Access blocked"}
                </p>
              </div>
              <div className="flex items-center gap-2.5">
                <span className={["text-xs font-semibold", isActive ? "text-emerald-400" : "text-red-400"].join(" ")}>
          ...
          <Section title="Subscription">
            ...
            <div className={["flex items-center gap-2 text-sm font-medium", subColor].join(" ")}>
              <span className={[
                "h-2 w-2 rounded-full",
                daysLeft === null ? "bg-gray-700"
        ...
        <div className="space-y-5 lg:col-span-3">
                  ...
                  return (
                    <div
                      ...
                      className={[
                        ...
                        isActive  ? "border-violet-500/60 bg-violet-950/30"
                        : isAllowed ? "border-gray-700 bg-gray-800/40"
                    ...
                    >
                      ...
                      {/* Set active */}
                      {isActive ? (
                        <span className="shrink-0 rounded bg-violet-700/40 px-2 py-0.5 text-xs font-semibold text-violet-300">
                          Active
                        </span>
                      ) : (
                        <button
                          onClick={() => setActive(d._id)}
                          className="shrink-0 rounded border border-gray-700 px-2 py-0.5 text-xs text-gray-500 hover:border-violet-600 hover:text-violet-400 transition-colors"
                        >
                          Set Active
                        </button>
                      )}
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <Input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addTid(); } }}
              ...
              <Button
                onClick={addTid}
                disabled={!tidInput.trim() || !tidNameInput.trim() || tids.includes(tidInput.trim())}
                className="h-9 w-full bg-gray-700 text-sm font-medium text-white hover:bg-gray-600 disabled:opacity-40">
                Add Tournament
              </Button>
            </div>
            {tids.length === 0 ? (
              <p className="text-xs text-gray-600">No tournaments assigned — user cannot use the controller.</p>
            ) : (
              <div className="space-y-1.5">
                {tids.map((tid) => (
                  <div key={tid}
                    ...
                    <div className="flex-1 min-w-0">
                      {tournamentNames[tid] && (
                        <p className="text-xs font-medium text-gray-300 truncate">{tournamentNames[tid]}</p>
                      )}
                      <p className="font-mono text-xs text-gray-500">{tid}</p>
                    </div>
                    <button onClick={() => removeTid(tid)}
                      className="shrink-0 text-gray-600 hover:text-red-400 transition-colors">×</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <Section title="Widget Access">
            <div className="rounded border border-gray-800 bg-gray-950/60 px-4 py-3">
              <p className="text-[10px] font-semibold tracking-widest text-gray-600 uppercase">User ID</p>
              <p className="mt-1 font-mono text-sm text-violet-400">{user._id}</p>
            </div>
            {tids.length === 0 ? (
              <p className="text-xs text-gray-500">Add tournament IDs above to generate display URLs.</p>
            ) : (
              <div className="space-y-1.5">
                <p className="text-[10px] font-semibold tracking-widest text-gray-600 uppercase">Display URLs</p>
                {tids.map((tid) => {
                    ...
                    <div key={tid}
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-medium text-gray-400">
                          {tournamentNames[tid] ? `${tournamentNames[tid]} · ` : ""}{tid}
                        </p>
                        <p className="truncate font-mono text-xs text-gray-500">{path}</p>
                      </div>
                      <button
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove <span className="font-semibold text-white">{user.name}</span> and all
...
function Section({ title, children }) {
  return (
    <div className="rounded border border-gray-800 bg-gray-900/60 p-5">
      <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-500">{title}</h2>
