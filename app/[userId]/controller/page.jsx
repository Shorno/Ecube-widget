...
export default function ControllerPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`tournamentId_${userId}`) ?? "";
    ...
    if (saved) {
      fetch(`/api/sse/state?userId=${userId}&tournamentId=${saved}`)
        .then((r) => r.json())
        .then(({ url, label }) => {
    ...
    const es = new EventSource(`/api/sse?userId=${userId}&tournamentId=${saved || ""}`);
  ...
  async function sendCommand(url, label) {
    ...
    try {
      const res = await fetch("/api/sse/command", {
        ...
        body: JSON.stringify({ url, label, userId, tournamentId: tournamentId.trim() }),
      });
      if (res.ok) {
        setActiveUrl(url);
  ...
  function handleSave() {
    ...
    if (!trimmed) return;
    localStorage.setItem(`tournamentId_${userId}`, trimmed);
    setIsSaved(true);
  }
      ...
      <header className="border-b border-gray-700 bg-gray-800 px-5 py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <Link
              href={`/${userId}/settings/design`}
              className="border border-gray-600 px-3 py-1.5 text-xs font-bold tracking-wider text-gray-400 uppercase hover:border-purple-500 hover:text-purple-400"
            >
              Theme
            </Link>
          </div>
