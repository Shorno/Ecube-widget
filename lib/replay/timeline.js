...
// Stable team standings from the completed recording. Replay controls use
// these final placements so the default top-four selection never changes as
// the operator seeks through the match.
export function finalTeamStandings(timeline) {
  const finalSnapshot = timeline.get("totalmessage")?.at(-1)?.body;
  if (!finalSnapshot) return [];

  const teamsById = new Map(
    (finalSnapshot.TeamInfoList ?? []).map((team) => [
      String(team.teamId),
      { ...team, rank: null },
    ]),
  );

  for (const player of finalSnapshot.TotalPlayerList ?? []) {
    ...
    if (rank > 0 && (team.rank === null || rank < team.rank)) {
      team.rank = rank;
    }
    teamsById.set(key, team);
  }
...
export function buildTeamLogoLookup(teamStandings) {
  return Object.fromEntries(
    teamStandings.map((team) => [String(team.teamId), team.logoPicUrl ?? null]),
  );
}
...
// PUBG-style zone countdown from the setcircleinfo phase machine. The phase and
// countdown math live in circleTimerFromInfo (shared with the live widgets);
// here we just anchor to the surrounding event's capture time so the countdown
// stays smooth at any playback speed.
export function circleTimerAt(timeline, time) {
  const info = eventAt(timeline, "setcircleinfo", time);
  if (!info) return null;
  return circleTimerFromInfo(info.body, (time - info.t) / 1000);
}
